...
$tweaks = @(
	...
	#"InstallVCRedist2015plusx64",
	#"InstallVCRedist2015plusx86",
	...
	"UninstallMicrosoftEdgeUpdate",
	"UninstallMicrosoftEdgeWebView2Runtime",
	"UninstallCortana",
	"UninstallMicrosoftNews",
	"UninstallMSNWeather",
	"UninstallGetHelp",
	"UninstallMicrosoftTips",
	"UninstallMicrosoft365Office",
	...
	"DisableFileExplorerCompactMode",
	...
	"AutomaticallyActiveHours",
	"DisableWindowsLatestUpdate",
	"WindowsTerminalDefaultTerminalApp",
	"DisableSATADrivesRemovableMedia",
	...
	"EnableAuditProcess",
	"EnableCommandLineProcessAudit",
	"EnableEventViewerCustomView",
	...
	"ShowMSIExtractContext",
	"ShowCABInstallContext",
	"HideRunAsDifferentUserContext",
	"HideCastToDeviceContext",
	"HideShareContext",
	"HideEditWithClipchampContext",
	"HidePrintCMDContext",
	"HideIncludeInLibraryContext",
	"HideSendToContext",
	"HideCompressedFolderNewContext",
	"EnableMultipleInvokeContext",
	"HideUseStoreOpenWith",
	"HideOpenWindowsTerminalContext",
	"EnableWindows10ContextMenu",

	# Other

	"DefaultTaskbarSize",
	"AlwaysCombineHideLabelsTaskbar",
	"LargeIconsControlPanelView",
...
Function InstallFTTProgs {
    ...
    if (Test-Path $wingetPath) {
        try {
            $currentVersion = & $wingetPath --version 2>$null
            Write-Output "Winget found. Version: $currentVersion"
            # Get latest version number from Microsoft Store (static URL, since winget updates are distributed via Store)
            # We'll use the package info for comparison, but winget doesn't provide an official API for latest version
            # So we attempt to upgrade to ensure latest version
            Write-Output "Checking for winget updates..."
            $upgradeResult = & $wingetPath upgrade --id Microsoft.DesktopAppInstaller -e --accept-source-agreements --accept-package-agreements
            if ($upgradeResult -match 'No applicable update found') {
                Write-Output "Winget is up to date."
            } else {
                Write-Output "Winget was updated or update attempted."
            }
        } catch {
            Write-Output "Error checking winget version. Will attempt to reinstall."
            $needsInstall = $true
        }
    } else {
    ...
    if ($needsInstall) {
        ...
        try {
            $nid = (Get-Process AppInstaller -ErrorAction Stop).Id
            Wait-Process -Id $nid
        } catch {
            Write-Output "AppInstaller process not found or already finished."
        }
        Write-Output "Winget Installed or Reinstalled."
    }
}
...
Function UninstallMicrosoft365Office {
	Write-Output "Trying to uninstall Microsoft 365 (Office)"
	winget uninstall --id 9WZDNCRD29V9	
}

# Install Microsoft 365 (Office)
Function InstallMicrosoft365Office {
	Write-Output "Trying to install Microsoft 365 (Office)"
	winget install --id 9WZDNCRD29V9
}
...
Function InstallMicrosoftSolitaireCollection {
	...
	winget install --id Microsoft.MicrosoftSolitaireCollection_8wekyb3d8bbwe
}
...
# Uninstall Windows Sound Recorder
Function UninstallWindowsSoundRecorder {
	Write-Output "Trying to uninstall Windows Sound Recorder"	
	winget uninstall "Windows Sound Recorder"	
}

# Install Windows Sound Recorder
Function InstallWindowsSoundRecorder {
	Write-Output "Trying to install Windows Sound Recorder"	
	winget install "Windows Sound Recorder"	
}
...
# Uninstall Game Bar
Function UninstallGameBar {
	Write-Output "Trying to uninstall Game Bar"
	winget uninstall "Game Bar"	
}

# Install Game Bar
Function InstallGameBar {
	Write-Output "Trying to install Game Bar"
	winget install "Game Bar"	
}
...
Function UninstallWindowsMediaPlayer {
	Write-Output "Trying to uninstall Windows Media Player"
	winget uninstall "Windows Media Player"	
}

# Install Windows Media Player
Function InstallWindowsMediaPlayer {
	Write-Output "Trying to install Windows Media Player"
...
# Do not get tip and suggestions when using Windows
...
Function HideBrowsingHistory {
	Write-Output "Hiding Browsing History ..."
...
Function ShowBrowsingHistory {
	Write-Output "Showing Browsing History ..."
	if ((Get-CimInstance -ClassName CIM_OperatingSystem).BuildNumber -lt 23451)
	{
		return
	}
...
Function HideCopilotButton {
	Write-Output "Hiding Copilot Button ..."
...
Function ShowCopilotButton {
	Write-Output "Showing Copilot Button ..."
...
Function AlwaysTaskbarCombine {
	Write-Output "Combining taskbar buttons and always hiding labels ..."
...
Function FullTaskbarCombine {
	Write-Output "Combining taskbar buttons and hiding labels when taskbar is full ..."
...
Function NeverTaskbarCombine {
	Write-Output "Combining taskbar buttons and never hiding labels ..."
	New-ItemProperty -Path HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced -Name taskbarGlomLevel -PropertyType DWord -Value 1 -Force
}
...
Function EnableRestartDeviceAfterUpdate {
	Write-Output "Enabling Restart Device AfterUpdate ..."
