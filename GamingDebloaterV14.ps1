# original from: https://freetimetech.com/gaming-debloater/
...
function Windows11Extra {
	If ([System.Environment]::OSVersion.Version.Build -ge 22000) {
		...
		Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "TaskbarAl" -Type DWord -Value 1 #set taskbar icons to the left
...
Function DisableHPET {
	...
	# Robust HPET enable: search by partial name, fallback if not found
	$hpet = Get-CimInstance -ClassName Win32_PnPEntity | Where-Object { $_.Name -like '*High*precision*event*timer*' }
	if ($hpet) {
		foreach ($dev in $hpet) {
			try {
				Enable-PnpDevice -InstanceId $dev.PNPDeviceID -Confirm:$false -ErrorAction Stop | Out-Null
				Write-Output "Enabled HPET device: $($dev.PNPDeviceID)"
			} catch {
				Write-Output "Could not enable HPET device: $($dev.PNPDeviceID). Try enabling manually in Device Manager."
			}
		}
	} else {
		Write-Output "HPET device not found on this system."
	}
	if ($PlatformCheck -eq "Desktop") {
...
Function NvidiaTweaks {
	...
	if(($CheckGPU -like "*GTX*") -or ($CheckGPU -like "*RTX*")) {
		...
		# Download the latest Nvidia Profile Inspector release from GitHub
		$inspectorZip = "$Env:windir\system32\nvidiaProfileInspector.zip"
		# $inspectorExe = "$Env:windir\system32\nvidiaProfileInspector.exe"
		$latestUrl = "https://github.com/Orbmu2k/nvidiaProfileInspector/releases/latest/download/nvidiaProfileInspector.zip"
		Invoke-WebRequest -Uri $latestUrl -OutFile $inspectorZip -ErrorAction SilentlyContinue
		# Extract the exe from the zip
		Add-Type -AssemblyName System.IO.Compression.FileSystem
		# Extract using PowerShell's Expand-Archive to avoid encoding issues
		Expand-Archive -Path $inspectorZip -DestinationPath "$Env:windir\system32\" -Force
		Remove-Item $inspectorZip -ErrorAction SilentlyContinue
		Push-Location
...
# Ask User if they want to enable Or disable Windows Defender
...
# Ask User if they want to enable Or disable Microsoft Software Protection Platform Service
Function askMSPPS {
	do
	{
		Clear-Host
...
#Ask User if they want to enable or disable Microsoft Store and WSAPPX Service
