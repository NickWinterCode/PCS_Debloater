...
#choco install adobereader --params "/DesktopIcon" -y
AcroRdrDCx642500120577_MUI.exe /norestart /quiet ALLUSERS=1 EULA_ACCEPT=YES  DISABLE_ARM_SERVICE_INSTALL=1

Write-Output "Installing VLC..."
choco install vlc -y

Write-Output "Installing Firefox..."
choco install Firefox --params "/l:de-DE" -y
...
# PowerShell's registry provider doesn't provide a method of preserving variable references, and we don't want to accidentally overwrite them with absolute path values.
# Where the registry allows us to see "%SystemRoot%" in a PATH entry, PowerShell's registry provider only sees "C:\Windows", for example.
...
$warningMessage = @"
This could cause issues after reboot where nothing is found if something goes wrong. In that case, look at the backup file for the original PATH values in '$backupFile'.
"@
...
if ($userPath -like "*$env:ChocolateyInstall*") {
    ...
    $newUserPATH = @(
        $userPath -split [System.IO.Path]::PathSeparator | Where-Object { $_ -and $_ -ne "$env:ChocolateyInstall\bin" }
...
if ($machinePath -like "*$env:ChocolateyInstall*") {
    ...
    Write-Warning $warningMessage
    $newMachinePATH = @(
        $machinePath -split [System.IO.Path]::PathSeparator | Where-Object { $_ -and $_ -ne "$env:ChocolateyInstall\bin" }
    ) -join [System.IO.Path]::PathSeparator
    # NEVER use [Environment]::SetEnvironmentVariable() for PATH values; see https://github.com/dotnet/corefx/issues/36449
    # This issue exists in ALL released versions of .NET and .NET Core as of 12/19/2019
