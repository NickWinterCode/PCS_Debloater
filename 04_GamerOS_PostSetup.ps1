...
#"SetTaskbarCombineNever",          #"SetTaskbarCombineWhenFull",    # 
...
"SetControlPanelLargeIcons",    #"SetControlPanelSmallIcons",
...
Function MinimalProcesses {
    ...
    ;Multitasking
    [HKEY_CURRENT_USER\Control Panel\Desktop]
    ""WindowArrangementActive""=dword:00000001
    ...
    Function StartMenuTiles {
        ...
        if ($null -eq $key) {
            Write-Host "Start Menu tile registry key not found. Skipping tile reset."
            return
        }
    
        # If $key is an array (multiple matches), use the first one
        if ($key -is [array]) {
            $key = $key[0]
        }
    
        if ($null -eq $key.Data) {
            Write-Host "Registry key found, but has no Data property. Skipping tile reset."
            return
        }
    ...
    Function RemoveEdgeBrowser {
        Write-Output "Removing Edge Browser... ERROR"
